...
public class MyPostionActivity extends BaseActivity {
    ...
    public static void gotoMyPostion(Context mContext, String title, String address,
                                     String lon, String lat) {
        Intent intent = new Intent(mContext, MyPostionActivity.class);
        intent.putExtra(MyPostionActivity.LON_KEY, lon);
        intent.putExtra(MyPostionActivity.LAT_KEY, lat);
        intent.putExtra(MyPostionActivity.ADDRESS_KEY, address);
        intent.putExtra(MyPostionActivity.TITLE_KEY, title);
        mContext.startActivity(intent);
    }
    ...
    protected void onCreate(Bundle arg0) {
        ...
        mBaseLibTopbarView.recovery().setLeftImageButton(R.drawable.icon_back, UIHelper.finish(_activity)).setTitle("地图")
                .setLeftText("返回", null);
        ...
        System.out.println("gotoMyPostion--map_lon" + lon);
        System.out.println("gotoMyPostion--map_lat" + lat);
        System.out.println("gotoMyPostion--address" + address);
        if (TextUtils.isEmpty(title)) {
        } else {
            mBaseLibTopbarView.setTitle(title);
        }
        mposition = (RelativeLayout) LayoutInflater.from(this).inflate(R.layout.item_map_ioc1, null);
    ...
    public void onClick(View arg0) {
        super.onClick(arg0);
        if (arg0.getId() == R.id.navigation) {
            mNavigationManager.gotoBaiduMap();
        } else if (arg0.getId() == R.id.location) {
...
//                            location.getLongitude());
                    ...
                    mNavigationManager.setPosition(curlat, curlon, lat, lon, address);
    ...
    private void initMap() {
        ...
        System.out.println("FDDataUtils.getDouble(lon)" + FDDataUtils.getDouble(lon));
        System.out.println("FDDataUtils.getDouble(lat)" + FDDataUtils.getDouble(lat));
        LatLng point = new LatLng(FDDataUtils.getDouble(lat), FDDataUtils.getDouble(lon));
        Builder mbuild = new Builder();
        mbuild.target(point);
        mbuild.zoom(15);
        MapStatusUpdate mapStatusUpdate = MapStatusUpdateFactory.newMapStatus(mbuild.build());
        mBaiduMap.setMapStatus(mapStatusUpdate);
        BitmapDescriptor bitmap = BitmapDescriptorFactory.fromView(mposition.findViewById(R.id.map_ioc_layout));
