...
public class MapFragment extends BaseFragment implements OnGetPoiSearchResultListener {
    ...
    public void changeState(int index) {
        ...
        for (int i = 0; i < currentClustersForItembean.size(); i++) {
            ...
            TextView number_tx = (TextView) itemView.findViewById(R.id.number_tx);
          ...
          /*  if (currentClustersForItembean.get(i).getCount() > 1) {
            ...
            } else if (currentClustersForItembean.get(i).getCount() == 1) {
                number_tx.setVisibility(View.GONE);
                ac.setImage(img, currentClustersForItembean.get(i).getPic());
            } else if (currentClustersForItembean.get(i).getCount() > 1) {
    ...
    private void initMark() {
        ...
        for (int i = 0; i < itembeanList.size(); i++) {
            ...
            if (TextUtils.isEmpty(itembean.getPic())) {
                ...
                marks.setPic(ServerConstant.BASEFILEURL + itembean.getPic());
            }
...
//            img.setImageResource(getIconId(false));
            ...
            System.out.println("地图渲染的图片地址：：：：getPic=" + currentClustersForItembean.get(i).getPic());
    ...
    public void getImage(String url) {
        ...
        ac.imageLoader.displayImage(url, new ImageView(_activity), new SimpleImageLoadingListener() {
            ...
            public void onLoadingComplete(String arg0, View arg1, Bitmap arg2) {
                ...
                if (currentClustersForItembean != null) {
                    for (int i = 0; i < currentClustersForItembean.size(); i++) {
                        if (currentClustersForItembean.get(i).getPic().equals(arg0)) {
                            System.out.println("currentClustersForItembean.get(i).getPic(::::::" + currentClustersForItembean.get(i).getPic().equals(arg0));
//                            View itemView = mapView.createView(i, currentClustersForItembean.get(i).getCount(), arg2);
                            View mView = mapView.createDefaultView(i, currentClustersForItembean.get(i).getCount());
                            TextView title = (TextView) mView.findViewById(R.id.title);
                            ImageView img = (ImageView) mView.findViewById(R.id.ioc_img);
                            TextView number_tx = (TextView) mView.findViewById(R.id.number_tx);
//                            initBitmapMarkView(title, number_tx, img, currentClustersForItembean.get(i).getName(),arg2, currentClustersForItembean.get(i).getCount());
                            BitmapDescriptor bdA = BitmapDescriptorFactory.fromView(mView);
     ...
     */
    private void getNetData(String left_top_lon, String left_top_lat, String right_bottom_lon, String right_bottom_lat) {
        if (mMapFragmentListener != null)
            mMapFragmentListener.getNetData(left_top_lon, left_top_lat, right_bottom_lon, right_bottom_lat);
    }
