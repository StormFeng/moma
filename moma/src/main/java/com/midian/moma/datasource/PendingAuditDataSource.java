 ...
 *
 */
public class PendingAuditDataSource extends BaseListDataSource<MyorderContent> {

    public PendingAuditDataSource(Context context) {
        super(context);
    }
    ...
    protected ArrayList<MyorderContent> load(int page) throws Exception {
        ...
        this.page = page;
        String type = "3";//0：全部，1：待付款，2：待提交，3待审核，4已完成
        MyOrdersBean bean = AppUtil.getMomaApiClient(ac).myOrders("3", page+"", AppContext.PAGE_SIZE);
        if (bean != null) {
            if (bean.isOK()) {
