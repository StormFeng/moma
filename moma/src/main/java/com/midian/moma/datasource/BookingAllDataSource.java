 ...
 *
 */
public class BookingAllDataSource extends BaseListDataSource<BookingContent> {

	public BookingAllDataSource(Context context) {
		super(context);
	}
    ...
    protected ArrayList<BookingContent> load(int page) throws Exception {
        ...
        this.page = page;
        String type = "0";//0：全部，1：待审核，2：已通过，3已拒绝
        MyBookingListBean bean = AppUtil.getMomaApiClient(ac).myReserves("0", this.page+ "", AppContext.PAGE_SIZE);
        if (bean != null) {
            if (bean.isOK()) {
