 ...
 *
 */
public class BookingThroughDataSource extends BaseListDataSource<BookingContent> {

	public BookingThroughDataSource(Context context) {
		super(context);
	}
    ...
    protected ArrayList<BookingContent> load(int page) throws Exception {
        ...
        this.page = page;
        String type = "2";//0：全部，1：待审核，2：已通过，3已拒绝
        MyBookingListBean bean = AppUtil.getMomaApiClient(ac).myReserves("2", this.page+ "", AppContext.PAGE_SIZE);
        if (bean != null) {
            if (bean.isOK()) {
